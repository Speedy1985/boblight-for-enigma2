...
{
  printf("\nBoblight Daemon - Boblight for enigma2 %s (c) 2012-2014 Speedy1985 and Oktay Oeztueter)\n",PACKAGE_VERSION);
  ...
  command << "ps aux | grep boblightd | grep -v 'grep' | grep -v 'gdb' | grep -v " << pid;
  int isRuning = system(command.str().c_str());
  if (isRuning == 0) {
      cout << "Boblightd is already running, Exit...." << endl;
      exit(1);
  }
