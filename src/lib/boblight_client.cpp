 ...
 * 
 * - Parts used from encoders library http://code.google.com/p/stringencoders/wiki/NumToA for faster float to string conversion.
 *
...
void CBoblight::AddBitmap(unsigned char* bmp, int xsize, int ysize, unsigned char delay)
{
  int rgb[3];
  int offset = 0;
  unsigned char shift;
...
int CBoblight::SetAdjust(int *adjust)
{
  string error;
  string data;
  bool   send;
